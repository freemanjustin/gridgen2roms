...
The last line in the input file, `geographic 148 -20`, informs `gridgen-c` that the polygon control points are in geographic (spherical) coordinates. The format is `geographic lon0 lat0` where `lon0`, `lat0` is the location of the South Pole for the stereographic projection. It should be some point in the region of the centre of the grid. The supplied web app will report the location of the centroid of your polygon which you may use as parameters for the `geographic` parameter. More information about `gridgen-c` is available by running `gridgen -h` on the command line.
...
```
where `polygon.grid` is the output generated by `gridgen-c`, `exmaple.xml` is the input XML paramter file for `gridgen2roms` and `roms_grid.nc` is the name of the output roms grid file created by `gridgen2roms`. In general, `gridgen2roms` has the following command line args structure

```
./gridgen2roms [gridgen output grid] [input file] [output file name for the roms grid (netcdf)]
```
...
### Polygon Upper Left Corner Assignment
You can "mark" a corner as the upper-left corner of the grid by putting "*" at the end of its line in the polygon control points file.
### Spline
...
```
ncks -d lat,-40.0,-5.0 -d lon,100.0,170.0 original_dataset.nc -o cutout.nc
```
### Ease of use
